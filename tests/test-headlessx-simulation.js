...
class HeadlessXSimulationTester {
  ...
  async runSimulation() {
    ...
    console.log(
      "Simulando el flujo completo de scraping y detección de cambios"
    );
    console.log("");
  ...
  async initialize() {
    ...
    console.log(
      `✅ Usuario ${created ? "creado" : "encontrado"}: ${user.name}`
    );
    ...
    console.log(
      `✅ Competidor ${competitorCreated ? "creado" : "encontrado"}: ${
        competitor.name
      }`
    );
  ...
  async loadHTMLFiles() {
    ...
    console.log(
      `✅ Total de versiones HTML cargadas: ${this.htmlVersions.length}`
    );
    console.log("");
  }
  ...
  async captureChangeVersion(htmlVersion, versionNumber) {
    ...
    if (!previousHtml) {
      ...
      previousHtml =
        this.htmlVersions[previousSnapshot.versionNumber - 1]?.html;
    }
    ...
    const changes = this.calculateDetailedChanges(
      previousHtml,
      htmlVersion.html
    );
    ...
    const shouldBeFullVersion = this.shouldCreateFullVersion(
      versionNumber,
      changes
    );
  ...
  calculateDetailedChanges(oldHtml, newHtml) {
    ...
    if (removedLines > 0)
      summaryParts.push(`${removedLines} líneas eliminadas`);

    // Analizar tipos de cambios
    const changeTypes = this.analyzeChangeTypes(differences);
    if (changeTypes.structure > 0)
      summaryParts.push(`${changeTypes.structure} cambios estructurales`);
    if (changeTypes.content > 0)
      summaryParts.push(`${changeTypes.content} cambios de contenido`);
    if (changeTypes.style > 0)
      summaryParts.push(`${changeTypes.style} cambios de estilo`);
  ...
  async verifyVersioning() {
    ...
    try {
      ...
      if (currentSnapshots.length !== 1) {
        throw new Error(
          `Debe haber exactamente 1 versión actual, hay ${currentSnapshots.length}`
        );
      }
      ...
      fullVersions.forEach((s) => {
        console.log(
          `   - Versión ${s.versionNumber}: ${
            s.fullHtml ? s.fullHtml.length : 0
          } caracteres`
        );
      });
    ...
    try {
      ...
      allSnapshots.forEach((snapshot) => {
        if (snapshot.isFullVersion && snapshot.fullHtml) {
          ...
          const estimatedSize =
            this.htmlVersions[snapshot.versionNumber - 1]?.html.length * 0.2 ||
            0;
      ...
      const theoreticalMaxStorage = this.htmlVersions.reduce(
        (acc, version) => acc + version.html.length,
        0
      );
      const efficiency = Math.round(
        (1 - totalStorage / theoreticalMaxStorage) * 100
      );

      console.log("💾 Análisis de almacenamiento:");
      console.log(
        `   - Almacenamiento actual: ~${Math.round(totalStorage)} caracteres`
      );
      console.log(
        `   - Almacenamiento máximo teórico: ${theoreticalMaxStorage} caracteres`
      );
      console.log(`   - Eficiencia: ${efficiency}%`);
      console.log(
        `   - Versiones completas: ${Math.round(fullVersionStorage)} caracteres`
      );
      console.log(
        `   - Versiones diferenciales: ~${Math.round(
          diffVersionStorage
        )} caracteres`
      );
      ...
      allSnapshots.forEach((snapshot) => {
        ...
        console.log(
          `   V${snapshot.versionNumber}: ${snapshot.changeCount} cambios (${
            snapshot.changePercentage
          }%) - ${snapshot.severity} - ${originalVersion?.file || "N/A"}`
        );
      });
  ...
  showSummary() {
    ...
    const totalPassed = this.testResults.filter(
      (r) => r.status === "PASS"
    ).length;
    const totalFailed = this.testResults.filter(
      (r) => r.status === "FAIL"
    ).length;
    ...
    if (totalPassed === totalTests) {
      ...
      console.log(
        "✅ El sistema de detección de cambios funciona correctamente"
      );
      console.log("");
      ...
      console.log(
        "💡 Revisa los errores específicos arriba para solucionarlos"
      );
    }
